...
contract("EMO", async accounts => {
  ...
  it("Resolve true claim by timeout", async () => {
    let initialStateHash = await client._seedToInitialStateHash(seed); //TODO add function to OR to calculate initialStateHash
    let tx = await client.makeClaim(seed, image, commitmentRoot, {value: stake});
    try {
    ...
    let claim = await verifier.getClaim(commitmentRoot);
    checkClaimRemoved(claim);

  });

  it("Falsify incorrect claim - go to Bottom", async () => {
    let initialStateHash = await client._seedToInitialStateHash(seed); //TODO add function to OR to calculate initialStateHash
    image.sum = 78;
    ...
    assert.equal(defendandTx.logs[0].args.prosecutorRoot, commitmentRoot, 'prosecutorRoot should match.');
    ...
    assert.equal(dispute.disagreementPoint, 0, "Not implemented yet.");
    ...
    assert.equal(dispute.goRight, true, "left nodes of the prosecutor and defendant nodes should be equal.");
    assert.equal(dispute.disagreementPoint, 0, "Not implemented yet.");
    assert.equal(dispute.depth, 3, "We should go deeper into the tree to the next level, depth should be 2 now.");
    assert.equal(dispute.state, 2, "should be 'ProsecutorTurn'.");

    // HERE we have the case where we are already at a Bottom of the tree and nodes are actually leaves and there is no way for prosecutor to act somehow to falsify this claim
  ...
  it("Resolve true claim that was disputed", async () => {

  });

});

function checkClaimRemoved(claim) {
